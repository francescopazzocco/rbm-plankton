"""
nan_test_eval.py - NaN test set evaluation for all RBM model families (L=6).
...
Outputs (all in results/nan_eval_extended/):
...
class EvalConfig:
    ...
    out_root:    Path   = field(default_factory=lambda:
                            Path(__file__).parent.parent / "results" / "nan_eval_extended")
    n_samples:   int    = 100
    impute_base: int    = 5
