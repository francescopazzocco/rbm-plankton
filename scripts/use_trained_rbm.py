...
OVERALL_METRICS = (
    ("reconstruction_error", "Reconstruction Error", "L2 error", False),
    ("reconstructed_distance", "Reconstructed Distance", "L2 distance", False),
...
def reconstruct_with_chain(model, visible, steps: int | None):
    if steps is None:
        with torch.no_grad():
            return model.reconstruct(visible.unsqueeze(0)).squeeze(0)
    return reconstruct_chain(model, visible, steps)
...
def load_evaluation_split(visible_model: str, device: torch.device, shuffle: bool):
    if visible_model.startswith('bernoulli'):
        X_train, X_val, dates_train, dates_val, taxa_cols, nan_rows, thresholds = \
            data_io.load_and_binarise(device=device, shuffle=shuffle)
        return X_val.to(device), taxa_cols, thresholds, dates_val
    X_train, X_val, dates_train, dates_val, taxa_cols, nan_rows = \
        data_io.load_raw_counts(device=device, shuffle=shuffle)
    return X_val.to(device), taxa_cols, None, dates_val


def evaluate_species_metrics(
    model,
    visible_model: str,
    X_val: torch.Tensor,
    taxa_cols: list[str],
    ...
    for i in range(D):
        ...
        for s in range(X_val.shape[0]):
            ...
            for _ in range(reps):
                V = orig.clone()
                V[i] = 0.0
                rec = reconstruct_with_chain(model, V, cd)
                rec = rec.detach()
        ...
        cos_masked_arr = np.asarray(cos_masked, dtype=float)
        cos_other_arr = np.asarray(cos_others, dtype=float)

        recon_means.append(float(errs_arr.mean()) if errs_arr.size > 0 else 0.0)
        recon_stds.append(float(errs_arr.std(ddof=0) / np.sqrt(errs_arr.size)) if errs_arr.size > 0 else 0.0)
        dist_means.append(float(dists_arr.mean()) if dists_arr.size > 0 else 0.0)
        dist_stds.append(float(dists_arr.std(ddof=0) / np.sqrt(dists_arr.size)) if dists_arr.size > 0 else 0.0)
        cos_masked_means.append(float(cos_masked_arr.mean()) if cos_masked_arr.size > 0 else 0.0)
        cos_masked_stds.append(float(cos_masked_arr.std(ddof=0) / np.sqrt(cos_masked_arr.size)) if cos_masked_arr.size > 0 else 0.0)
        cos_other_means.append(float(cos_other_arr.mean()) if cos_other_arr.size > 0 else 0.0)
        cos_other_stds.append(float(cos_other_arr.std(ddof=0) / np.sqrt(cos_other_arr.size)) if cos_other_arr.size > 0 else 0.0)

    return {
        'reconstruction_error': {
            'means': np.asarray(recon_means, dtype=float),
            'sems': np.asarray(recon_stds, dtype=float),
            'score': float(np.sum(recon_means)),
        },
        'reconstructed_distance': {
            'means': np.asarray(dist_means, dtype=float),
            'sems': np.asarray(dist_stds, dtype=float),
            'score': float(np.sum(dist_means)),
        },
        'cosine_masked': {
            'means': np.asarray(cos_masked_means, dtype=float),
            'sems': np.asarray(cos_masked_stds, dtype=float),
            'score': float(np.sum(cos_masked_means)),
        },
        'cosine_other': {
            'means': np.asarray(cos_other_means, dtype=float),
            'sems': np.asarray(cos_other_stds, dtype=float),
            'score': float(np.sum(cos_other_means)),
        },
    }
...
def aggregate_family_l_results(run_entries: list[dict]):
    ...
    for entry in run_entries:
        ...
        for metric_key, _, _, higher_is_better in OVERALL_METRICS:
            stacked_means = np.stack([seed_entry['metrics'][metric_key]['means'] for seed_entry in seed_entries], axis=0)
            metric_mean = stacked_means.mean(axis=0)
            if stacked_means.shape[0] > 1:
                metric_sem = stacked_means.std(axis=0, ddof=1) / np.sqrt(stacked_means.shape[0])
            else:
                metric_sem = np.zeros_like(metric_mean)
            metric_bundle[metric_key] = {
                'means': metric_mean,
                'sems': metric_sem,
                'score': float(metric_mean.sum()),
...
def select_top_runs(aggregated_runs: list[dict], metric_key: str):
    higher_is_better = next(cfg[3] for cfg in OVERALL_METRICS if cfg[0] == metric_key)
    ranked = sorted(
        aggregated_runs,
        key=lambda entry: entry['metrics'][metric_key]['score'],
        reverse=higher_is_better,
    )
    return ranked[:3]


def plot_overall_summary(overall_runs: dict[str, list[dict]], taxa_cols: list[str], out_path: Path):
    ...
    for ax, (metric_key, title, ylabel, higher_is_better) in zip(axes, OVERALL_METRICS):
        top_runs = overall_runs[metric_key]
        for idx, entry in enumerate(top_runs):
            color = cmap(idx % 10)
            label = f"{entry['family']} | L={entry['L']}"
            if metric_key == 'reconstruction_error':
                label += f" (score={entry['metrics'][metric_key]['score']:.2f})"
            elif metric_key == 'reconstructed_distance':
                label += f" (score={entry['metrics'][metric_key]['score']:.2f})"
            else:
                label += f" (score={entry['metrics'][metric_key]['score']:.2f})"
            ax.errorbar(
                x,
                entry['metrics'][metric_key]['means'],
                yerr=entry['metrics'][metric_key]['sems'],
                fmt='-o',
                linewidth=1.5,
                markersize=3,
                color=color,
    ...
    print(f"[Eval] Saved overall plots to {out_path}")


def serialize_overall_results(overall_runs: list[dict], selected_top: dict[str, list[dict]], out_path: Path):
    payload = {
        'runs': [],
        'top_3': {},
    }
    for entry in overall_runs:
        run_payload = {
            'family': entry['family'],
            'model_name': entry['model_name'],
            'L': entry['L'],
        ...
        for seed_entry in entry['seed_entries']:
            run_payload['seed_entries'].append({
                'seed': seed_entry['seed'],
                'metrics': {
                    metric_key: {
                        'means': np.asarray(metric_data['means']).tolist(),
                        'sems': np.asarray(metric_data['sems']).tolist(),
                        'score': metric_data['score'],
                    }
                    for metric_key, metric_data in seed_entry['metrics'].items()
                },
            })
        for metric_key, metric_data in entry['metrics'].items():
            run_payload['metrics'][metric_key] = {
                'means': np.asarray(metric_data['means']).tolist(),
                'sems': np.asarray(metric_data['sems']).tolist(),
                'score': metric_data['score'],
            }
    ...
    for metric_key, entries in selected_top.items():
        payload['top_3'][metric_key] = [
            {'family': entry['family'], 'L': entry['L'], 'score': entry['metrics'][metric_key]['score']}
            for entry in entries
        ]

    out_path.parent.mkdir(parents=True, exist_ok=True)
    out_path.write_text(json.dumps(payload, indent=2), encoding='utf-8')
    print(f"[Eval] Saved overall results to {out_path}")
...
def main():
    ...
    p.add_argument('--overall', action='store_true', help='Evaluate all families, hidden sizes, and seeds, then plot top-3 models per metric')
    p.add_argument('--plot-out', type=str, default=None, help='Output PNG path for plots')
    args = p.parse_args()

    if args.overall:
        device = torch.device('cuda' if (args.device == 'cuda' and torch.cuda.is_available()) else 'cpu')
        print('\n[Overall] Running cross-family imputation evaluation')
        if args.CD is not None:
            print(f'[Overall] Chained reconstruction enabled: CD={args.CD}')

        family_records = []
        taxa_cols_ref = None
        for model_name, label in MODEL_FAMILIES:
            if model_name == 'bernoulli_median' or model_name == 'bernoulli_zero':
                visible_model = model_name
            elif model_name.startswith('nb'):
                visible_model = model_name
            elif model_name.startswith('zinb'):
                visible_model = model_name
            else:
                visible_model = model_name

            X_val, taxa_cols, _, _ = load_evaluation_split(visible_model, device, args.shuffle)
            taxa_cols_ref = taxa_cols_ref or taxa_cols

            for hidden_units in OVERALL_HIDDEN_UNITS:
                seed_entries = []
                for seed in OVERALL_SEEDS:
                    weights_path = ROOT / 'training_runs' / f'{model_name}_L{hidden_units}_shuffled' / f'seed_{seed}' / 'weights.npz'
                    if not weights_path.exists():
                        print(f"[Overall] skip {label} L={hidden_units} seed={seed}: missing weights")
                        continue

                    npz = load_weights_npz(weights_path)
                    model, loaded_visible_model = instantiate_model_from_weights(npz, device_str=args.device, override_class=args.class_name)
                    if loaded_visible_model != visible_model:
                        visible_model = loaded_visible_model

                    metrics = evaluate_species_metrics(
                        model,
                        visible_model,
                        X_val.to(device),
                        taxa_cols,
                    ...
                    print(
                        ...
                        f"recon_score={metrics['reconstruction_error']['score']:.4f}, "
                        f"dist_score={metrics['reconstructed_distance']['score']:.4f}, "
                        f"cos_other_score={metrics['cosine_other']['score']:.4f}"
                    )
        ...
        aggregated_runs = aggregate_family_l_results(family_records)
        selected_top = {metric_key: select_top_runs(aggregated_runs, metric_key) for metric_key, _, _, _ in OVERALL_METRICS}

        if args.plot_out is None:
            out_path = ROOT / 'results' / 'reconstruction_plots' / 'use_trained_rbm_overall.png'
        else:
            out_path = Path(args.plot_out)
            out_path.parent.mkdir(parents=True, exist_ok=True)

        plot_overall_summary(selected_top, taxa_cols_ref or taxa_cols, out_path)
        serialize_overall_results(
            aggregated_runs,
            selected_top,
            out_path.with_name('use_trained_rbm_overall_results.json'),
        )
        return
